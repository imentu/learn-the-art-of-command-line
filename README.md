...
- 学会如何使用 `man` 命令去阅读文档。学会使用 `apropos` 去查找文档。知道有些命令并不对应可执行文件，而是在 Bash 内置好的，此时可以使用 `help` 和 `help -d` 命令获取帮助信息。你可以用 `type 命令` 来判断这个命令到底是可执行文件、shell 内置命令还是别名。[NOTE：帮助手册](notes/help-manual.md)
...
在[原始作品](https://github.com/jlevy/the-art-of-command-line/blob/master/README-zh.md)的基础上增加个人学习笔记。
